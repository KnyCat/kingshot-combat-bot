...
def extract(
    ...
    for power_token in powers:
        row_positions = [position for position in edge_positions if 35 <= power_token["center_y"] - position["center_y"] <= 110]
        position_value = int(min(row_positions, key=lambda token: abs(power_token["center_y"] - token["center_y"] - 70))["digits"]) if row_positions else 0
        name_candidates = [
            token for token in selected
            if re.search(r"[^\W\d_]", token["text"], re.UNICODE)
            and 12 <= power_token["center_y"] - token["center_y"] <= 55
        ...
        if position_value:
            last_position[0] = max(last_position[0], position_value)
            if name_key:
            ...
            position_value = min(20, last_position[0] + 1)
            last_position[0] = position_value
            known_positions[name_key] = position_value
        else:
            continue
